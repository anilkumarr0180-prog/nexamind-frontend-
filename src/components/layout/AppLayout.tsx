...
export const AppLayout = () => {
          ...
          return (
            <Link
              ...
              className={`group relative flex items-center justify-between rounded-xl px-3 py-2 text-xs transition-all duration-150 ${
                isCurrent
                  ? 'bg-surface-elevated text-slate-100 font-medium border border-white/[0.07] shadow-subtle'
            ...
            >
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 hover:text-rose-400 p-1 text-slate-400 hover:bg-rose-500/10 rounded transition-all flex-shrink-0 cursor-pointer"
                title="Delete chat"
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col justify-between bg-surface border-r border-subtle select-none">
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-subtle">
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-600 text-white shadow-subtle group-hover:bg-brand-500 transition-colors">
              <svg
          ...
          <button
            onClick={() => setMobileMenuOpen(false)}
            className="md:hidden rounded-lg p-1.5 text-slate-400 hover:text-slate-200 hover:bg-surface-hover transition-colors"
        ...
        <div className="p-3">
          <button
            ...
            className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl bg-surface-elevated hover:bg-surface-hover active:bg-surface-active text-slate-200 hover:text-white text-xs font-medium border border-subtle hover:border-brand-500/30 transition-all shadow-subtle group cursor-pointer"
          >
            <div className="flex items-center gap-2">
              <svg
                className="w-4 h-4 text-brand-400 group-hover:text-brand-300 transition-colors"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
        ...
        <div className="px-2 flex-1 min-h-0 overflow-y-auto">
          {isLoadingConversations ? (
            <div className="space-y-2 p-3">
              <div className="h-5 bg-surface-muted rounded animate-pulse w-3/4" />
              <div className="h-5 bg-surface-muted rounded animate-pulse w-5/6" />
              <div className="h-5 bg-surface-muted rounded animate-pulse w-2/3" />
            </div>
        ...
        <div className="px-2 py-2 border-t border-subtle space-y-0.5">
          <NavLink
            to="/app/memories"
            onClick={() => setMobileMenuOpen(false)}
            className={({ isActive }) =>
          ...
          >
            ...
            {activeMemoriesCount > 0 && (
              <span className="text-[10px] text-brand-300 bg-brand-500/15 border border-brand-500/25 px-1.5 py-0.2 rounded-full font-mono">
                {activeMemoriesCount}
              </span>
            )}
          </NavLink>
      ...
      <div className="p-3 border-t border-subtle">
        <div className="flex items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="h-7 w-7 rounded-full bg-brand-600/30 border border-brand-500/30 flex items-center justify-center text-xs font-semibold text-brand-200 flex-shrink-0">
          ...
          <button
            ...
            title="Sign out"
            aria-label="Sign out"
            className="rounded-lg p-1.5 text-slate-400 hover:text-slate-200 hover:bg-surface-hover transition-colors flex-shrink-0 cursor-pointer"
          >
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-canvas text-slate-100 font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-canvas">
        {/* Top Header */}
        <header className="h-14 border-b border-subtle bg-surface/40 backdrop-blur-md flex items-center justify-between px-4 sm:px-6 z-10 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            {/* Mobile menu trigger */}
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="md:hidden rounded-lg p-1.5 text-slate-400 hover:text-white hover:bg-surface-hover transition-colors flex-shrink-0"
            ...
            {/* Title */}
            <h1 className="text-sm font-medium text-slate-200 tracking-tight truncate">
              {getHeaderTitle()}
            </h1>
          </div>
              ...
              <>
                {activeConvId && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors cursor-pointer"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        strokeLinecap="round"
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-slate-300 hover:text-white bg-surface-elevated hover:bg-surface-hover border border-subtle rounded-lg transition-colors cursor-pointer"
                >
